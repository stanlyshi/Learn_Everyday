...
[root@localhost 1]# ./compile.sh latency.c  -DTEST_SIZE=128 -DTEST_NLOOP=100000
[root@localhost 1]# ./a.out 
[-289159424]Latency 0.206406 ms.
[-289159424]Latency 0.113023 ms.
[-755783936]Latency 0.221893 ms.
[-755783936]Latency 0.097419 ms.
...
#define log_send(fmt...)  do{printf("\033[33m[%d]", TID);printf(fmt);printf("\033[m");}while(0)
#define log_recv(fmt...)  do{printf("\033[32m[%d]", TID);printf(fmt);printf("\033[m");}while(0)
...
void test_mqueue_send(send_hook_fn send_fn)
{
    ...
    while(++nloop <= TEST_NLOOP) {
        ...
        gettimeofday(&send_pdmsg->tv, NULL);
...
void test_mqueue_recv(recv_hook_fn recv_fn)
{
    ...
    while(++nloop <= TEST_NLOOP) {
        ...
        if(n <= 0) {
            ...
            unsigned long int usec = diff_timeval_usec(&tv, &pdmsg->tv);
            totalusec += usec;
...
//                        n, recv_bytes, pdmsg->msg_seq, pdmsg->msg_ack, pdmsg->msg_len);
    ...
    gettimeofday(&end, NULL);
    log_recv("Latency %lf ms.\n", totalusec*1.0/(nloop-1)/1000.0);
