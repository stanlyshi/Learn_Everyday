...
all: xtlib
	...
	mv xt_GTPU.ko $(BIN_DIR)/

clean: libclean
	make -C /lib/modules/$(KVERSION)/build M=$(PWD) clean

################################################################################
# The next section is used by libxt_GTPU
################################################################################
...
LIB_CFLAGS := -fPIC -Wall -MMD -D_LARGEFILE_SOURCE=1 -D_LARGE_FILES -D_FILE_OFFSET_BITS=64 \
          ...
          -D_INIT=libxt_GTPU_init -DPIC -fPIC -O2 \
...
$(XT_BUILD_DIR)/%.o : %.c
	@echo "Building GTPu ext for version $(IPTVERSION) $(IPTABLES_VERSION_STRING)"
	@echo Compiling $<
