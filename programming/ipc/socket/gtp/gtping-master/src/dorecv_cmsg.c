...
/**
 * 
 *//*  内核空间的接口         [rongtao comment 2019.10.17]*/
ssize_t doRecv(int sock, void *data, size_t len, int *ttl, int *tos)
{
	...
	if (options.verbose > 2) {
        ...
        *tos = -1;
        ...
        n = recvmsg(sock, &msgh, MSG_WAITALL);/*   [rongtao comment 2019.10.17]*/

        for (cmsg = CMSG_FIRSTHDR(&msgh);/* 为什么用到内核的接口，还有用户空间的fprintf？？？？   [rongtao comment 2019.10.17]*/
             (0 < n) && (cmsg != NULL);
