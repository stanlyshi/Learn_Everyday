...
{
	struct msghdr msg;/*   [rongtao comment 2019.10.17]*/
	struct cmsghdr *cmsg;
	char cbuf[512];
	char buf[5120];
	...
	if (0 > (n = recvmsg(fd, &msg, MSG_ERRQUEUE))) {/*   [rongtao comment 2019.10.17]*/
		if (errno == EAGAIN) {
                        goto errout;
		}
	...
	for (cmsg = CMSG_FIRSTHDR(&msg);
	     cmsg;
	     cmsg = CMSG_NXTHDR(&msg, cmsg)) {/*   [rongtao comment 2019.10.17]*/
