...
setupSocket()
{
	...
	if (addrs->ai_family == AF_INET6) {/*    [rongtao comment 2019.10.17]*/
                int on = 1;
		if (options.ttl > 0) {
	...
	/* connect() *//*   [rongtao comment 2019.10.17]*/
	if (connect(fd,
		    addrs->ai_addr,
		    addrs->ai_addrlen)) {/*   [rongtao comment 2019.10.17]*/
		err = errno;
 ...
 *
 */
static size_t mkping_v1(int seq, void **packet)
{
        ...
        *packet = gtp;
        ...
        gtp->seq = htons(seq);/*   [rongtao comment 2019.10.17]*/
        gtp->npdu = 0x00;
        gtp->next = 0x00;
 ...
 *
 */
static size_t mkping_v2(int seq, void **packet)/*   [rongtao comment 2019.10.17]*/
{
 ...
 *
 */
static size_t mkping(int seq, void **packet)/*   [rongtao comment 2019.10.17]*/
{
        switch (options.version) {
        case 1:
                return mkping_v1(seq, packet);/*   [rongtao comment 2019.10.17]*/
        case 2:
                return mkping_v2(seq, packet);/*   [rongtao comment 2019.10.17]*/
        }
        fprintf(stderr,
 ...
 *//*   [rongtao comment 2019.10.17]*/
static int sendEcho(int fd, int seq)
{
	int err = 0;
        void *packet = 0;
        ssize_t packetlen;
	...
	if (options.verbose > 2) {
        ...
        if (0 > (packetlen = mkping(seq, &packet))) {/*   [rongtao comment 2019.10.17]*/
                err = packetlen;
                goto errout;
        }
	...
	if (packetlen != send(fd, packet, packetlen, 0)) {/*   [rongtao comment 2019.10.17]*/
		err = errno;
		if (err == ECONNREFUSED) {
 ...
 *
 */
static struct GtpReply parseReply_v1(const void *packet, size_t packetlen)
{/*   [rongtao comment 2019.10.17]*/
        struct GtpReply ret;
 ...
 *
 */
static struct GtpReply parseReply_v2(const void *packet, size_t packetlen)
{/*   [rongtao comment 2019.10.17]*/
        struct GtpReply ret;
 ...
 *
 */
static struct GtpReply parseReply(const void *packet, size_t packetlen)
{/*   [rongtao comment 2019.10.17]*/
        ...
        switch (gtp->version) {
        case 1:
                return parseReply_v1(packet, packetlen);/*   [rongtao comment 2019.10.17]*/
        case 2:
                return parseReply_v2(packet, packetlen);/*   [rongtao comment 2019.10.17]*/
        }
...
recvEchoReply(int fd)
{
	...
	memset(packet, 0, sizeof(packet));
        if (0 > (packetlen = doRecv(fd,/*   [rongtao comment 2019.10.17]*/
                                    (void*)packet,
                                    sizeof(packet),
                                    &ttl,
                                    &tos))) {
		switch(errno) {
                case ECONNREFUSED:
                        connectionRefused++;
			handleRecvErr(fd, "Port closed", 0);/*   [rongtao comment 2019.10.17]*/
                        return 1;
		case EINTR:
                        return 1;
                case EHOSTUNREACH:
        ...
        gtp = parseReply(packet, packetlen);/*   [rongtao comment 2019.10.17]*/
        if (!gtp.ok) {
                return 1;
        }
 ...
 * with pingMainloop()
 *//*   [rongtao comment 2019.10.17]*/
static int tracerouteMainloop(int fd)/*   [rongtao comment 2019.10.17]*/
{
        int ttl = 0;
        int ttlTry = 0;
        double curPingTime;
	...
	while (!sigintReceived) {
		struct pollfd fds;/*   [rongtao comment 2019.10.17]*/
		...
		if ((lastRecvTime >= lastPingTime)
                    || (curPingTime > lastPingTime + options.interval)) {
                        ...
                        if (0 <= sendEcho(fd, curSeq++)) {/*   [rongtao comment 2019.10.17]*/
		...
		case 1: /* read ready */
			...
			if (fds.revents & POLLERR) {
                                int e;
				e = handleRecvErr(fd, NULL, lastPingTime);/*   [rongtao comment 2019.10.17]*/
                                if (e) {
			...
			if (fds.revents & POLLIN) {
				n = recvEchoReply(fd);/*   [rongtao comment 2019.10.17]*/
                                endOfTraceroute = 1;
                                if (!n) {
                                        lastRecvTime = clock_get_dbl();
                                } else if (n > 0) {
 ...
 */
static int pingMainloop(int fd)
{
	unsigned sent = 0;
	unsigned recvd = 0;
	...
	while (!sigintReceived) {
		...
		if (curPingTime > lastpingTime + options.interval) {
			...
			} else if (0 <= sendEcho(fd, curSeq++)) {/*   [rongtao comment 2019.10.17]*/
                                sent++;
                                lastpingTime = curPingTime;
                                if (options.flood) {
                                        printf(".");
 ...
 *//*   [rongtao comment 2019.10.17]*/
static int string2Tos(const char *instr)
{
...
main(int argc, char **argv)
{
	...
	if (0 > (fd = setupSocket())) {/*   [rongtao comment 2019.10.17]*/
		return 1;
	}
        if (options.traceroute) {
                return tracerouteMainloop(fd);/*   [rongtao comment 2019.10.17]*/
        } else {
                return pingMainloop(fd);/*   [rongtao comment 2019.10.17]*/
        }
}
