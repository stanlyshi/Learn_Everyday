...
#ifdef HAVE_UINT64_T
#define U8TO64_LE(p) 							\
    ((sip_uint64_t)U8TO32_LE(p) | ((sip_uint64_t)U8TO32_LE((p) + 4)) << 32 )
...
static inline void
u64to8_le(uint8_t *p, sip_uint64_t v)
{
    U32TO8_LE(p,     v.lo);
    U32TO8_LE(p + 4, v.hi);
}
...
static inline sip_uint64_t *
rotl64_to(sip_uint64_t *v, unsigned int s)
{
...
static inline sip_uint64_t *
add64_to(sip_uint64_t *v, const sip_uint64_t s)
{
    v->lo += s.lo;
    v->hi += s.hi;
    if (v->lo < s.lo) v->hi++;
...
static inline sip_uint64_t *
xor64_to(sip_uint64_t *v, const sip_uint64_t s)
{
    v->lo ^= s.lo;
    v->hi ^= s.hi;
    return v;
...
typedef struct {
  ...
  int d;
  sip_uint64_t v[4];
  uint8_t buf[sizeof(sip_uint64_t)];
  uint8_t buflen;
  uint8_t msglen_byte;
} sip_state;
...
typedef struct {
    ...
    void (*final)(sip_state *s, sip_uint64_t *digest);
} sip_interface;
...
static void int_sip_final(sip_state *state, sip_uint64_t *digest);
...
int_sip_init(sip_state *state, uint8_t key[16])
{
    ...
    k0 = U8TO64_LE(key);
    k1 = U8TO64_LE(key + sizeof(sip_uint64_t));
...
static inline void
int_sip_update_block(sip_state *state, sip_uint64_t m)
{
...
int_sip_pre_update(sip_state *state, uint8_t **pdata, size_t *plen)
{
    ...
    to_read = sizeof(sip_uint64_t) - state->buflen;
...
{
    uint8_t r = len % sizeof(sip_uint64_t);
    if (r) {
...
int_sip_update(sip_state *state, uint8_t *data, size_t len)
{
    ...
    data64 = (sip_uint64_t *) data;

    int_sip_pre_update(state, &data, &len);

    end = data64 + (len / sizeof(sip_uint64_t));
	...
	sip_uint64_t m;
	uint8_t *data8 = data;
	for (; data8 != (uint8_t *) end; data8 += sizeof(sip_uint64_t)) {
	    m = U8TO64_LE(data8);
	    int_sip_update_block(state, m);
	}
    }
...
int_sip_pad_final_block(sip_state *state)
{
    ...
    for (i = state->buflen; i < sizeof(sip_uint64_t); i++) {
	state->buf[i] = 0x00;
    }
    state->buf[sizeof(sip_uint64_t) - 1] = state->msglen_byte;
}

static void
int_sip_final(sip_state *state, sip_uint64_t *digest)
{
    sip_uint64_t m;
...
sip_hash_final(sip_hash *h, uint8_t **digest, size_t* len)
{
    ...
    if (!(ret = (uint8_t *)malloc(sizeof(sip_uint64_t)))) return 0;
    U64TO8_LE(ret, digest64);
    *len = sizeof(sip_uint64_t);
    *digest = ret;
...
int
sip_hash_final_integer(sip_hash *h, sip_uint64_t *digest)
{
...
int
sip_hash_digest_integer(sip_hash *h, uint8_t *data, size_t data_len, sip_uint64_t *digest)
{
    if (!sip_hash_update(h, data, data_len)) return 0;
...
sip_hash24(uint8_t key[16], uint8_t *data, size_t len)
{
    ...
    sip_uint64_t m, last;
    uint8_t *end = data + len - (len % sizeof(sip_uint64_t));
    ...
    {
        sip_uint64_t *data64 = (sip_uint64_t *)data;
        while (data64 != (sip_uint64_t *) end) {
	    m = *data64++;
	    SIP_2_ROUND(m, v0, v1, v2, v3);
        }
    }
#elif BYTE_ORDER == BIG_ENDIAN
    for (; data != end; data += sizeof(sip_uint64_t)) {
	m = U8TO64_LE(data);
	SIP_2_ROUND(m, v0, v1, v2, v3);
    }
#endif
...
#ifdef HAVE_UINT64_T
    last = len << 56;
#define OR_BYTE(n) (last |= ((sip_uint64_t) end[n]) << ((n) * 8))
#else
    last.hi = len << 24;
    last.lo = 0;
#define OR_BYTE(n) do { \
    ...
    switch (len % sizeof(sip_uint64_t)) {
	case 7:
	    OR_BYTE(6);
	case 6:
	    OR_BYTE(5);
...
#if BYTE_ORDER == LITTLE_ENDIAN && UNALIGNED_WORD_ACCESS
  #if HAVE_UINT64_T
	    last |= (sip_uint64_t) ((uint32_t *) end)[0];
  #else
