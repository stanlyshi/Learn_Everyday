...
int sip_hash_final_integer(sip_hash *h, sip_uint64_t *digest);
int sip_hash_digest(sip_hash *h, uint8_t *data, size_t data_len, uint8_t **digest, size_t *digest_len);
int sip_hash_digest_integer(sip_hash *h, uint8_t *data, size_t data_len, sip_uint64_t *digest);
void sip_hash_free(sip_hash *h);
void sip_hash_dump(sip_hash *h);

sip_uint64_t sip_hash24(uint8_t key[16], uint8_t *data, size_t len);
