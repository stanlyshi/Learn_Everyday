...
static CURLcode sslctx_function(CURL *curl, void *sslctx, void *parm)
{
  ...
  static const char mypem[] =
    ...
    "-----END CERTIFICATE-----\n";
  //内存BIO是将内存用于其I / O 的源/接收BIO。
  //写入存储器BIO的数据存储在 BUF_MEM结构中，该结构会适当扩展以容纳存储的数据。
  BIO *cbio = BIO_new_mem_buf(mypem, sizeof(mypem));
  
  //获取X509_STORE   SSL_CTX结构中的结构。X509_STORE结构  保留用于证书验证的信息
  X509_STORE  *cts = SSL_CTX_get_cert_store((SSL_CTX *)sslctx);//在以下位置获取X509_STORE结构
  int i;
...
int main(void)
{
  ...
  curl_easy_setopt(ch, CURLOPT_URL, "http://10.170.6.66/");
